...
export async function callUpdate(callUpdate) {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        const callmsg = await mconn.conn.reply(nk.from, `Hello *@${nk.from.split('@')[0]}*, this ${nk.isVideo ? 'video calls' : 'and calls'} are not allowed, you will be blocked.\n-\nIf you accidentally called contact my creator to unblock you!`, false, {mentions: [nk.from]});
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        unreg: '*[ 🛑 𝐇𝐄𝐘!!📇Register to use this feature by Typing:\n\n*/reg name.age*\n\n📌Example : */reg *LORD ZEUS-MD*༆𝐁𝐎𝐓.17*',
        restrict: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓  ⚠️ ] 🔐 This feature is *disabled*',
  }[type];
  const aa = {quoted: m, userJid: conn.user.jid};
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '[ ⚠ ] ༆𝗔𝗟𝗘𝗥𝗧 - 𝗔𝗟𝗘𝗥𝗧༆', body: '༆ᴛʜᴇ *LORD ZEUS-MD* ʙᴏᴛ༆', thumbnail: imagen1, sourceUrl: 'https://github.com/LORD ZEUS-MD'}}}}, aa);

...
*WRITE THE ROW AND COLUMN NUMBER FROM THE BEGINNING OF THE FIRST LETTER _"${PALABRA.charAt(0)}"_ OF THE WORD_"${PALABRA}"_ HAS _${intentos}_ ATTEMPTS!!*

*EXAMPLE:*
❇️ \`\`\`${usedPrefix + command} 28\`\`\`
➡️ \`\`\`ROW 2\`\`\`    ⬇️ \`\`\`COLUMNA 8\`\`\``.trim())
...
await conn.reply(m.chat, ` *@${tagUser.split("@")[0]} HE IS CURRENTLY PLAYING LETTER SOUP 🔠*`, m, { mentions: [tagUser] })
return
}
if (intentos === 0) {
intentos = 3  
...
fila = null, columna = null, sopaNube = null, sopaPalabra = null, sopaDir = null, userSP = null, cambioLetra = null
intentos = 0
await m.reply(` 🫡 *YOU EXHAUSTED YOUR ATTEMPT!! THE WORD _"${sopaPalabra}"_ WAS AT THE ADDRESS _${cambioLetra}_ FROM THE ROW _${fila}_ AND COLUMN _${columna}_*`)
return  
} else {
intentos -= 1
await m.reply(` 😮‍💨 *INCORRECT. YOU ARE LEFT _${intentos}_ ATTEMPTS!!*${intentos === 1 ? '' : `\n*PALABRA A ENCONTRAR:* \`\`\`${sopaPalabra}\`\`\``}\n\n${intentos === 1 ? `\`\`\`💡 TRACK!!\`\`\`\n*THE WORD _${sopaPalabra}_ FOUND AT THE ADDRESS _"${cambioLetra}"_*\n\n` : ''}${sopaNube}`)
}}
}}
